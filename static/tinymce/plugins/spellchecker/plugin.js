	...
	PluginManager.add('spellchecker', function(editor, url) {
		...
		/*function defaultSpellcheckCallback(method, text, doneCallback, errorCallback) {
			...
			XHR.send({
				...
				url:"http://aa.joinf.com:8080/common/spellcheck",
				type: "post",
				dataType: "json",
				content_type: 'application/json',
				data: {"content":"The quck brwn dog jmped over the fnce"},
		...
		function defaultSpellcheckCallback(method, text, doneCallback, errorCallback) {
			var postData = {"content":text};
			 $.ajax({
	            type: "POST",
	            url: settings.spellcheck_url,
	            contentType: "application/json",
	            dataType: "json", 
	            data: JSON.serialize(postData),//'{"content":"The quck brwn dog jmped over the fnce"}',//相当于 //data: "{'str1':'foovalue', 'str2':'barvalue'}",
	            success: function (result) {
					if (!result.result) {
		              var message = editor.translate("Server response wasn't proper JSON.");
		              errorCallback(message);
		            } else {
		              doneCallback(result.data);//JSON.parse(result.data)
		            }
	            },
				error: function(type, xhr) {
					errorCallback("Spellchecker request error: " + xhr.status);
		...
		/**
		  * 格式化后端返回的值,转成和插件的格式一致
		  * @param {Object} result 后端返回格式
		  * @param {Object} newData 处理后的格式,如下:
		  * {
		  *     dictionary: false,
		  *     words: {
		...
		function formatData(result){
			...
			if(data){
				...
				for(var i=0,l=data.length; i<l; i++){
					newData.words[data[i].invalidWord] = [];
					var words = data[i].suggestedWord;
						
					for(var j=0,k=words.length; j<k; j++){
						newData.words[data[i].invalidWord].push(words[j].word);
					}
				}
			}else{
				newData = {
