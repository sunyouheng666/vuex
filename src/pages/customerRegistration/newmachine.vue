<template>
    <div class="pageBox newdog">
        ...
        <div style="padding-top:39px;" >
            ...
            <mt-field label="机器码：" placeholder="请输入机器码" v-model="machineCode"  :attr="{maxlength: 10 }">
                <span class="required">*</span>   
            </mt-field>
        ...
        <div class="btn-bottom">
            ...
            <mt-button  v-show="auditStatus==0&&titleName=='编辑机器码'"  type="primary" slot="right" @click.native="updateAuditStatus('1')" style="margin-right:10px;background:#5e67fc;">
                申请
            </mt-button> 
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            back: function() {                    //返回功能
                this.$router.push({path: 'customerRegistration',query:{type:'3'}});
            }, 
            ...
            validateForm: function() {              //市场日志表单验证
                ...
                if (this.machineCode == '') {
                    MessageBox('提示', '请输入机器码！');
                    return false
                }else{
                    var reg = /^[a-zA-Z0-9]{0,10}$/;
                    if (!reg.test(this.machineCode)) {
                        MessageBox('提示', '机器码输入错误(数字与字母)');
                        return false
                    }
                }
            ...
            insertDongle(){                 //新增加密狗
                ...
                axios.post(this.html_url +this.Apilist.insertMachine, param).then(function(data) {
                    MessageBox.alert('新增机器码成功！','提示'); 
                    me.back();
                }).catch(function(e) {});
            },
            updateDongle(){                 //修改加密狗
                ...
                axios.post(this.html_url +this.Apilist.updateMachine, param).then(function(data) {
                    MessageBox.alert('修改机器码成功！','提示'); 
                    me.back();
                }).catch(function(e) {});
            },
            updateAuditStatus(type){
                ...
                axios.post(this.html_url +this.Apilist.updateAuditStatus, param).then(function(data) {
                    if(type==1){
                        MessageBox.alert('客户机器码发送审核成功！','提示'); 
                    }else{
                        MessageBox.alert('客户机器码审核成功！','提示'); 
                    }
                    me.back();
                }).catch(function(e) {});
            },
...
<style>
    ...
    .newdog .mint-field .mint-cell-title{width: 80px!important;text-align: left;color: #2894e5;}
</style>