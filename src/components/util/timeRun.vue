  ...
  methods: {
    counterFn: function(counterTime) {
      ...
      if (vm.day > 0) {
        ...
        if (vm.minute < 10) {
          if (vm.second < 10) {
            vm.countString = `0${vm.minute}:0${vm.second}`;
          } else {
            vm.countString = `0${vm.minute}:${vm.second}`;
          }
        } else {
          if (vm.second < 10) {
            vm.countString = `${vm.minute}:0${vm.second}`;
          } else {
            vm.countString = `${vm.minute}:${vm.second}`;
          }
          
        }
      } else {
        if (vm.second < 10) {
          vm.countString = `00:0${vm.second}`;
        } else {
          vm.countString = `00:${vm.second}`;
        }
      }
    },
